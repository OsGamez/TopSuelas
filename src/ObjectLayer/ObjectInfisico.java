...
//    Connection server = db.tempphy();
    ...
    public boolean AddTemp(Infisico inv) {
        try {
            c.setAutoCommit(false);
            st = c.prepareStatement("INSERT INTO invtemp (Almacen,Producto,Estilo,Corrida,Combinacion,Tipo,Pto1,"
                    + "Pto2,Pto3,Pto4,Pto5,Pto6,Pto7,Pto8,Pto9,Pto10,Pto11,Pto12,Pto13,Pto14,TotalPares)"
                    + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            st.setInt(1, inv.getAlmacen());
            st.setInt(2, inv.getProducto());
    ...
    public boolean deleteTemp() {
        try {
            c.setAutoCommit(false);
            st = c.prepareStatement("DELETE FROM invtemp");
            st.executeUpdate();
            c.commit();
            st.close();
            return true;
    ...
    public ArrayList<Infisico> GetInventarioAgrupado() {
        ArrayList<Infisico> listaInv = new ArrayList<>();
        try {
            st = c.prepareStatement("SELECT Producto, Almacen, Estilo, Corrida, Combinacion,Tipo,\n"
                    + "sum(Pto1) as Pto1,sum(Pto2) as Pto2,sum(Pto3) as Pto3,sum(Pto4) as Pto4,sum(Pto5) as Pto5,sum(Pto6) as Pto6,sum(Pto7) as Pto7,\n"
                    + "sum(Pto8) as Pto8,sum(Pto9) as Pto9,sum(Pto10) as Pto10,sum(Pto11) as Pto11,sum(Pto12) as Pto12,sum(Pto13) as Pto13,\n"
                    + "sum(Pto14) as Pto14,sum(TotalPares) as TotalPares\n"
                    + "FROM invtemp group by Producto,Almacen,Estilo,Corrida,Combinacion,Tipo\n"
                    + "order by Producto");
            rs = st.executeQuery();
            while (rs.next()) {
                Infisico inv = new Infisico();
    ...
    public ArrayList<Infisico> GetCaptura(String nombre) {
        ArrayList<Infisico> listaInv = new ArrayList<>();
        try {
            st = c.prepareStatement("SELECT p.Descripcion as Suela,dt.Producto, dt.Almacen, dt.Estilo, dt.Corrida, dt.Combinacion,dt.Tipo,\n"
                    + "dt.Pto1,dt.Pto2,dt.Pto3,dt.Pto4,dt.Pto5,dt.Pto6,dt.Pto7,\n"
                    + "dt.Pto8,dt.Pto9,dt.Pto10,dt.Pto11,dt.Pto12,dt.Pto13,dt.Pto14,dt.TotalPares FROM " + nombre + " dt inner join RCPTPhylon..Productos p\n"
                    + "on dt.Producto = p.Producto");
